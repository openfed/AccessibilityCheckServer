...
export class ApiService {
   ...
   */
  private createObservableFromSocketIoEvent(
    eventName: string
  ): Observable<any> {
  ...
  getSniffResults(url: string): Observable<SniffResult> {
    ...
    return this.getAllSniffResults().filter(
      (result: SniffResult) => url === result.url
    );
  }
  ...
  getSniffLoading(url: string): Observable<SniffLoading> {
    return this.createObservableFromSocketIoEvent("sniff-loading").filter(
      (result: SniffLoading) => url === result.url
    );
  }
  ...
  getSniffError(url: string): Observable<SniffError> {
    return this.createObservableFromSocketIoEvent("sniff-error").filter(
      (result: SniffError) => url === result.url
    );
  }
